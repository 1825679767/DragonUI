...
local function OpenDragonUIPluginManagement()
    if addon.OptionsPanel and addon.OptionsPanel.Open then
        addon.OptionsPanel:Open("plugins")
        return
    end

    addon:ToggleOptionsUI("plugins")
end

local function KeepExternalMinimapButtonHidden(button)
    if not button or button.DragonUI_AlwaysHidden then
        return
    end

    button.DragonUI_AlwaysHidden = true
...
local function HideDBMMinimapButton()
    if DBM and DBM.Options then
        DBM.Options.ShowMinimapButton = false
        if type(DBM.HideMinimapButton) == "function" then
            pcall(DBM.HideMinimapButton, DBM)
        end
    end
...
local function HideSkadaMinimapButton()
    if Skada and Skada.db and Skada.db.profile and Skada.db.profile.icon then
        Skada.db.profile.icon.hide = true
        if type(Skada.RefreshMMButton) == "function" then
            pcall(Skada.RefreshMMButton, Skada)
        end
    end

    if LibStub and LibStub.GetLibrary then
        local ok, libDBIcon = pcall(LibStub.GetLibrary, LibStub, "LibDBIcon-1.0", true)
        if ok and libDBIcon and type(libDBIcon.Hide) == "function" then
            pcall(libDBIcon.Hide, libDBIcon, "Skada")
        end
    end

    local button = _G.LibDBIcon10_Skada
    if button then
        KeepExternalMinimapButtonHidden(button)
        button:Hide()
    end
...
local function HideIntegratedAddonMinimapButtons()
    HideDBMMinimapButton()
    HideSkadaMinimapButton()
end
