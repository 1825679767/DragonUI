...
local function GetClassDisplayName(classToken)
	if classToken == "GLOBAL" then
		return L["Global"]
	end

	return LOCALIZED_CLASS_NAMES_MALE[classToken]
		or LOCALIZED_CLASS_NAMES_FEMALE[classToken]
		or classToken
end
...
function classDropDown:initialize(level, menuList)
	...
	for _, thisClass in pairs(classes) do
		info.text = GetClassDisplayName(thisClass)
		info.arg1 = thisClass
...
local function ClassUpdate()
	...
	UIDropDownMenu_SetText(classDropDown, GetClassDisplayName(class))
