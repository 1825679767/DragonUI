...
local function OpenGatherMateConfig()
	if not Config:EnsureOptionsRegistered() then
		return false
	end

	local AceConfigDialog = LibStub("AceConfigDialog-3.0", true)
	if not AceConfigDialog and not IsAddOnLoaded("DragonUI_Options") then
...
function Config:OnInitialize()
	...
	self.optionButton:SetHeight(19);
	if IsAddOnLoaded("Mapster") then
		self.optionButton:SetPoint("TOPRIGHT", WorldMapPositioningGuide, "TOPRIGHT", -135, -2);
	else
		self.optionButton:SetPoint("TOPRIGHT", WorldMapPositioningGuide, "TOPRIGHT", -43, -2);
	end
	...
	if DataBroker then
		local launcher = DataBroker:NewDataObject("GatherMate", {
		    ...
		    OnClick = function(clickedframe, button) OpenGatherMateConfig() end,
		})
	end
end

function Config:EnsureOptionsRegistered()
	...
	if (not AceDBOptions or not AceConfigRegistry) and not IsAddOnLoaded("DragonUI_Options") then
		LoadAddOn("DragonUI_Options")
		AceDBOptions = LibStub("AceDBOptions-3.0", true)
		AceConfigRegistry = LibStub("AceConfigRegistry-3.0", true)
	end
	if not AceDBOptions or not AceConfigRegistry then
		return false
	end

	options.plugins["profiles"] = { profiles = AceDBOptions:GetOptionsTable(GatherMate.db) }
	self.options = options
	self.importHelper = ImportHelper
	AceConfigRegistry:RegisterOptionsTable("GatherMate", options)
