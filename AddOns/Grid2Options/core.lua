...
local Grid2Options = {
	options = {
		Auras = {
			type = "group",
			name = LG["Auras"] or "Auras",
			desc = L["Options for %s."]:format(LG["Auras"] or "Auras"),
			args = {},
		},
	},
	plugins = {},
...
function Grid2Options:LocalizeText(text)
	if type(text) ~= "string" then
		return text
	end
	return L[text] or LG[text] or text
end

function Grid2Options:GetDisplayName(element)
	if type(element) == "string" then
		return self:LocalizeText(element)
	end
	if not element then
		return nil
	end
	return self:LocalizeText(element.displayName or element.menuName or element.name)
end

function Grid2Options:GetDisplayDesc(element)
	return L["Options for %s."]:format(self:GetDisplayName(element))
end
...
function Grid2Options:AddModule(parent, name, module, extraOptions)
	...
	local displayName = self:GetDisplayName(module)

	local options = {
		type = "group",
		order = module.menuOrder or 400,
		name = displayName,
		desc = self:GetDisplayDesc(module),
		args = {},
	}
...
function Grid2Options:AddElement(elementType, element, extraOptions)
	...
	local options = {}
	local displayName = self:GetDisplayName(element)
	group.args[element.name] = {
		type = "group",
		name = displayName,
		desc = self:GetDisplayDesc(element),
		args = options,
	}
	for name, option in pairs(extraOptions) do
		options[name] = option
...
function Grid2Options:AddElementGroup(type, extraOptions, order, reset)
	...
	if (reset or not group) then
		local displayType = self:GetDisplayName(type)
		group = {
			type = "group",
			order = order,
			name = displayType,
			desc = L["Options for %s."]:format(displayType),
			args = {},
		}
...
function Grid2Options:AddElementSubTypeGroup(type, subType, subTypeOptions, reset)
	...
	if (reset or not subGroup) then
		local displaySubType = self:GetDisplayName(subType)
		subGroup = {
			type = "group",
			name = displaySubType,
			desc = L["Options for %s."]:format(displaySubType),
			args = options,
		}
