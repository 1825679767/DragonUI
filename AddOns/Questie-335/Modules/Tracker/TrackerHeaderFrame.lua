...
function TrackerHeaderFrame.Initialize(baseFrame)
    ...
    trackedQuests.label.GetUnboundedStringWidth = QuestieCompat.GetUnboundedStringWidth
    trackedQuests.label.GetWrappedWidth = trackedQuests.label.GetWidth
    trackedQuests.label.GetNumLines = QuestieCompat.GetNumLines
...
function TrackerHeaderFrame:Update()
    ...
    local trackedQuestsLabel = headerFrame.trackedQuests.label

    if trackedQuestsLabel and not trackedQuestsLabel.GetUnboundedStringWidth then
        trackedQuestsLabel.GetUnboundedStringWidth = QuestieCompat.GetUnboundedStringWidth
    end
    ...
    if Questie.db.profile.trackerHeaderEnabled or (not QuestieTracker:HasQuest()) then
        ...
        local labelWidth = trackedQuestsLabel and trackedQuestsLabel:GetUnboundedStringWidth() or 0

        trackedQuestsLabel:SetPoint("TOPLEFT", headerFrame.questieIcon, "TOPRIGHT", 2, 0)
        headerFrame.trackedQuests:SetWidth(labelWidth)
        ...
        headerFrame:SetWidth(labelWidth + trackerFontSizeHeader)
        headerFrame:SetHeight(trackerFontSizeHeader + 5)
        headerFrame:Show()

        TrackerHeaderFrame.PositionTrackerHeaderFrame(headerFrame, trackerBaseFrame)

        QuestieCompat.SetResizeBounds(trackerBaseFrame, labelWidth, trackerFontSizeHeader)
    else
        headerFrame:Hide()
